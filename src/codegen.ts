      ...
      for (const key in entity.properties) {
        ...
        switch (prop.type.kind) {
          ...
          case 'enum':
            addIndexAnnotation(entity, key, imports, out);
            out.line(
              `@Column_("varchar", {length: ${getEnumMaxLength(model, prop.type.name)}, nullable: ${prop.nullable}})`,
            );
            break;
          case 'fk':
          ...
          case 'union':
            ...
            out.line(
              `@Column_("jsonb", {transformer: {to: obj => ${marshalToJson(
                prop,
                'obj',
              )}, from: obj => ${marshalFromJson(prop, 'obj')}}, nullable: ${prop.nullable}})`,
            );
            break;
          case 'list':
            switch (prop.type.item.type.kind) {
              case 'scalar':
                out.line(
                  `@Column_("${getDbType(prop.type.item.type.name)}", {array: true, nullable: ${prop.nullable}})`,
                );
                break;
              case 'enum':
                out.line(
  ...
  function marshalToJson(prop: Prop, exp: string): string {
    ...
    switch (prop.type.kind) {
      ...
      case 'union':
        ...
        break;
      case 'list': {
        const marshal = marshalToJson(prop.type.item, 'val');
...
function addIndexAnnotation(entity: Entity, field: string, imports: ImportRegistry, out: Output): void {
  const index = getFieldIndex(entity, field);
...
function getFieldIndex(entity: Entity, field: string): { unique?: boolean } | undefined {
  ...
  const candidates = entity.indexes?.filter((index) => index.fields[0]?.name == field) || [];
  ...
  render(model: Model): string[] {
    ...
    for (const name of this.model) {
      switch (model[name].kind) {
        case 'entity':
          imports.push(`import {${name}} from "./${toCamelCase(name)}.model"`);
          break;
        default: {
          const names = [name];
          ...
          imports.push(`import {${names.join(', ')}} from "./_${toCamelCase(name)}"`);
        }
      }
    }
    return imports;
